...
import GameboyDesaturator.GameboyShader;
...
var bushes1:FlxSprite;
var hills1:FlxSprite;
var trees1:FlxSprite;
var sky1:FlxSprite;
...
var bushes2:FlxSprite;
var hills2:FlxSprite;
...
var emberEmitter:FlxEmitter;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			var fire:BGSprite = new BGSprite('lordpenis/Superstitionfire',0.9, 0.9);
			fire.updateHitbox();
			...
			camGame.setFilters([new ShaderFilter(filter2.shader)]);
			...
			var bush:BGSprite = new BGSprite('lordpenis/SuperstitionTrees', 0.7, 0.7);
			bush.updateHitbox();
			bush.setGraphicSize(Std.int(bush.width * 2));
			bush.screenCenter();
			bush.antialiasing = true;
			add(bush);
			
			var leaf:BGSprite = new BGSprite('lordpenis/SuperstitionBushes');
			leaf.updateHitbox();
			leaf.setGraphicSize(Std.int(leaf.width * 2));
			leaf.screenCenter();
			leaf.antialiasing = true;
			add(leaf);

			var ground:BGSprite = new BGSprite('lordpenis/Superstitiongrass');
			...
			for (i in 0 ... 150)
				...
				p.frames = Paths.getSparrowAtlas('lordpenis/ember');
			...
			ground2 = new FlxSprite().loadGraphic(Paths.image('silentBG/phase2/ground'));
			 ground2.scale.set(2.4, 2.4);
			 
			 bushes2 = new FlxSprite().loadGraphic(Paths.image('silentBG/phase2/bushes'));
			 bushes2.scale.set(2.4, 2.4);
			 bushes2.scrollFactor.set(1.1, 1.1);
			 
			 hills2 = new FlxSprite().loadGraphic(Paths.image('silentBG/phase2/hills'));
			 hills2.scale.set(2.4, 2.4);
			 hills2.scrollFactor.set(1.2, 1.2);
		 
			 trees2 = new FlxSprite().loadGraphic(Paths.image('silentBG/phase2/trees'));
			 trees2.scale.set(2.4, 2.4);
			 trees2.scrollFactor.set(1.05, 1.05);
		 
			 sky2 = new FlxSprite().loadGraphic(Paths.image('silentBG/phase2/sky'));
			 sky2.scale.set(2.4, 2.4);
			 sky2.scrollFactor.set(0.6, 0.7);
			 ...
			 caShader.amount = -0.5;
			 var filter2:ShaderFilter = new ShaderFilter(caShader.shader);
			 camGame.setFilters([new ShaderFilter(VignetteShade.shader),new ShaderFilter(filter2.shader)]);
			...
			case 'xalamussy':
			 ...
			 // Normal
			 ground1 = new FlxSprite().loadGraphic(Paths.image('silentBG/phase1/ground'));
			 ground1.scale.set(2.4, 2.4);
			 
			 bushes1 = new FlxSprite().loadGraphic(Paths.image('silentBG/phase1/bushes'));
			 bushes1.scale.set(2.4, 2.4);
			 bushes1.scrollFactor.set(1.1, 1.1);
			 
			 hills1 = new FlxSprite().loadGraphic(Paths.image('silentBG/phase1/hills'));
			 hills1.scale.set(2.4, 2.4);
			 hills1.scrollFactor.set(1.2, 1.2);
		 
			 trees1 = new FlxSprite().loadGraphic(Paths.image('silentBG/phase1/trees'));
			 trees1.scale.set(2.4, 2.4);
			 trees1.scrollFactor.set(1.05, 1.05);
		 
			 sky1 = new FlxSprite().loadGraphic(Paths.image('silentBG/phase1/sky'));
			 sky1.scale.set(2.4, 2.4);
			 sky1.scrollFactor.set(0.6, 0.7);

			 VignetteShade = new VignetteEffect();
			 shaderFilter.push(new ShaderFilter(VignetteShade.shader));
			 camGame.setFilters([new ShaderFilter(VignetteShade.shader)]);
		...
		timeTxt.setFormat(Paths.font("sonc.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("sonc.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("sonc.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			else
			{
                ...
                FlxG.switchState(new SongCompleteSub());
				changedDifficulty = false;
			}
			transitioning = true;
		}
	...
	override function stepHit()
	{
		...
		if(SONG.song.toLowerCase() == "devils-island") {
			switch(curStep) {
				case 896:
					camHUD.visible = false;
				...
				case 2078:
					  ...
					  trace('balls');
					  camGame.camera.fade(FlxColor.BLACK);
		...
		if(SONG.song.toLowerCase() == "silent-panic") {
			switch(curStep) {
				case 774:
				...
				case 789:
					camGame.flash(FlxColor.RED, 0.55);
				...
				case 795:
				  ...
				  timer = new FlxTimer().start(2, fuckyohealth, 0);  
					
				      if(ClientPrefs.mechanics == 'pussy')
						{
							timer.cancel(); 
				...
				case 1600:
				timer.cancel(); 
			  timer1 = new FlxTimer().start(1, fuckyohealthHARD, 0);  
	...
	private function fuckyohealth(timer:FlxTimer)
		{

			if(ClientPrefs.mechanics == 'moderate')
            {
			...
			if(ClientPrefs.mechanics == 'unbearable')
				{
					if(health > 0.7)
					{
						health -= 0.25;
		...
		private function fuckyohealthHARD(timer1:FlxTimer)
			{
				if(ClientPrefs.mechanics == 'moderate')
					{
						if(health > 0.7)
